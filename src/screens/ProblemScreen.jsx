      ...
      <div style={{ padding: "0 16px" }}>
            ...
            {submitted && step.why && (
              <div style={{ marginTop: 12 }}>
                ...
                {step.why.options.map((opt, i) => {
                  ...
                  if (whySubmitted) {
                    if (i === step.why.correct) { bg = "#052e16"; border = "#22c55e50"; col = "#22c55e"; icon = "✓"; }
                    else if (i === whyChoice && i !== step.why.correct) { bg = "#1c0000"; border = "#ef444450"; col = "#ef4444"; icon = "✗"; }
                  }
                  return (
                    <button key={i} onClick={() => !whySubmitted && selectWhy(i)} style={{
                      display: "flex", alignItems: "flex-start", gap: 8, width: "100%", textAlign: "left",
                      background: bg, border: `1px solid ${border}`, borderRadius: 8,
                      padding: "10px 12px", marginBottom: 7, color: col, fontSize: 13,
                      cursor: whySubmitted ? "default" : "pointer", lineHeight: 1.5,
                    }}>
                      <span style={{ flexShrink: 0, fontWeight: 700, minWidth: 18 }}>
                        {whySubmitted && icon ? icon : String.fromCharCode(65 + i) + "."}
                      </span>
                      <span style={{ flex: 1 }}>{opt}</span>
                    </button>
                  );
                ...
                {whySubmitted && (
                  <div style={{ fontSize: 13, color: "#c7d2e8", background: "#070d18", border: "1px solid #1e3a5e", borderRadius: 8, padding: "12px 14px", lineHeight: 1.7 }}>
                    <span style={{ color: "#60a5fa", fontWeight: 700 }}>💡 Explanation: </span>
                    {step.why.explanation}
                  </div>
                )}
              </div>
            )}
        ...
        {phase === "dryrun" && (
          <div>
            ...
            <MCQBlock data={problem.dryrun} choice={specialChoice} submitted={specialSubmitted} onSelect={selectSpecial} onNext={advancePhase} nextLabel="Interviewer Pushback →" accentColor="#22c55e" />
          </div>
        )}
        ...
        {phase === "pushback" && (
          <div>
            ...
            <MCQBlock data={problem.pushback} choice={specialChoice} submitted={specialSubmitted} onSelect={selectSpecial} onNext={advancePhase} nextLabel="Follow-up Question →" accentColor="#f59e0b" />
          </div>
        )}
        ...
        {phase === "followup" && (
          <div>
            ...
            <MCQBlock data={problem.followup} choice={specialChoice} submitted={specialSubmitted} onSelect={selectSpecial} onNext={advancePhase} nextLabel="See Results →" accentColor="#a78bfa" />
          </div>
        )}
      </div>
    </div>
